...
public class MainSocket {
		...
		try {
			...
			if(removePlayer != null)
			{
				...
				if(error.isSet())
				{
					 for(Session s : ConnectionManager.socketList)
					 {
						 sendErrorMessage(session, error.getDescription(), true, true);
					 }
	...
	private void handleSTG(JSONObject stgJson, Session session) throws IOException, JSONException
	{
		...
		Collection<Session> sessions = ConnectionManager.getSessions();
		for(Session s : sessions)
		{
			/*Player p = sessionPlayerLink.get(s);
			if(p != null) {
				System.out.println(p.getName() + " wurde zu Session " + s.getId() + " gefunden");
